...
namespace wipo.patches.EliteInCastle
{
    ...
    internal class AiVisitSettlementBehaviorPatch : CampaignBehaviorBase
    {

        [HarmonyPrefix]
        static bool Prefix(ref AiVisitSettlementBehaviorPatch __instance, MobileParty mobileParty, PartyThinkParams p)
        {
            ...
            if (mobileParty.Army == null || mobileParty.Army.LeaderParty == mobileParty || mobileParty.Army.Cohesion < (float)mobileParty.Army.CohesionThresholdForDispersion)
            {
                ...
                ValueTuple<float, float, int, int> valueTuple = __instance.CalculatePartyParameters(mobileParty);
                ...
                {
                    num7 = __instance.CalculateSellItemScore(mobileParty);
                }
                ...
                SortedList<ValueTuple<float, int>, Settlement> sortedList = __instance.FindSettlementsToVisitWithDistances(mobileParty);
                ...
                foreach (KeyValuePair<ValueTuple<float, int>, Settlement> keyValuePair2 in sortedList)
                {
                    ...
                    else
                    {
                        ...
                        if (item < 1f && mobileParty.CanPayMoreWage())
                        {
                            ...
                            {
                                num31 = (float)__instance.ApproximateNumberOfVolunteersCanBeRecruitedFromSettlement(leaderHero, value2);
                        ...
                        if (num16 >= 2.5f)
                        {
                            __instance.AddBehaviorTupleWithScore(p, value2, num16);
                            break;
                        }
                        float num39 = 1f;
                        if (num31 > 0f)
                        ...
                        if (num26 <= 0.5f)
                        {
                            ValueTuple<float, float, float, float> valueTuple2 = __instance.CalculateBeingSettlementOwnerScores(mobileParty, value2, currentSettlementOfMobilePartyForAICalculation, -1f, num19, item);
        ...
        private SortedList<ValueTuple<float, int>, Settlement> FindSettlementsToVisitWithDistances(MobileParty mobileParty)
        {
            ...
            if (mobileParty.LeaderHero != null && mobileParty.LeaderHero.MapFaction.IsKingdomFaction)
            {
                if (mobileParty.Army == null || mobileParty.Army.LeaderParty == mobileParty)
                {
                    ...
                    {
                        if (settlement.MapFaction != mobileParty.MapFaction && IsSettlementSuitableForVisitingCondition(mobileParty, settlement))
                        {
        ...
        public bool IsSettlementSuitableForVisitingCondition(MobileParty mobileParty, Settlement settlement)
        {
        ...
        private float CalculateMergeScoreForLeaderlessParty(MobileParty leaderlessParty, Settlement settlement, float distance)
        {
            ...
            if (leaderlessParty.ActualClan != settlement.OwnerClan)
            {
                ...
                num2 = (((actualClan?.MapFaction) == settlement.MapFaction) ? 0.35f : 0f);
            }
            else
            {
                num2 = 2f;
        ...
        private float CalculateMergeScoreForDisbandingParty(MobileParty disbandParty, Settlement settlement, float distance)
        {
            ...
            float num2;
            if ((owner?.Clan) != settlement.OwnerClan)
            {
                Hero owner2 = disbandParty.Party.Owner;
                num2 = (((owner2?.MapFaction) == settlement.MapFaction) ? 0.35f : 0.025f);
            }
            else
            {
                num2 = 1f;
